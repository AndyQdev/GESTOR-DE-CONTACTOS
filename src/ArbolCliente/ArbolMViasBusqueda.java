...
public class ArbolMViasBusqueda<K extends Comparable<K>, V> implements
    ...
    private int buscarPosicionPorDondeBajar(K claveAInsertar, NodoMVias<K, V> nodoActual) {
        for (int i = 0; i < nodoActual.nroDeClavesNoVacias(); i++) {
            if (claveAInsertar.compareTo(nodoActual.getClave(i)) < 0) {
                return i;
            }
    ...
    private void insertarClaveYValorOrdenado(NodoMVias<K, V> nodoActual, K claveAInsertar, V valorAsociado) {
        int pos = -1;
    ...
    //AYUDA PARA LAINTERFAZ
    
    
      public List<V> listarClientes() {
        List<V> lista = new ArrayList<>();
        listarClientes(this.raiz, lista);
        return lista;
    }

    private void listarClientes(NodoMVias nodo, List<V> lista) {
        if (nodo != null) {
            
            for (int i = 0; i < nodo.nroDeClavesNoVacias(); i++) {
            recorridoEnInOrden(nodo.getHijo(i), (List<K>) lista);
            lista.add((V) nodo.getValor(i));
        }
        recorridoEnInOrden(nodo.getHijo(nodo.nroDeClavesNoVacias()), (List<K>) lista);
          
        }
    }

 public boolean editarClienteAMV(K filaAEditar,V valorEditado) {
        if (!this.esArbolVacio()) {
        Queue<NodoMVias<K, V>> colaDeNodos = new LinkedList<>();
        colaDeNodos.offer(this.raiz);
        do {
            ...
            for (int i = 0; i < nodoActual.nroDeClavesNoVacias(); i++) {
                 ...
                 if(nodoActual.getClave(i).compareTo(filaAEditar)==0){
                     nodoActual.setValor(i, valorEditado);
                     return true;
                    }
                if (!nodoActual.esHijoVacio(i)) {
                    colaDeNodos.offer(nodoActual.getHijo(i));
